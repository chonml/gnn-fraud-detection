...
class HeteroRGCNLayerSachin(nn.Module):
    def __init__(self, in_size, out_size, etypes):
        super(HeteroRGCNLayer, self).__init__()
        self.weight = nn.ModuleDict({
            name: nn.Linear(in_size, out_size) for name in etypes
    ...
    def forward(self, G, feat_dict):
        ...
        # TODO: Implement Scaling Factor depending on node feature
        # HINT: Create a dictionary/hash of different node features and a floating point scaling factor
        # HINT: iterate through G.ntypes and if 'h' feature is present then apply .get(ntype, (whatever index of the dictionary)) and then multiply by that factor
