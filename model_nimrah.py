...
class HeteroRGCNLayerNimrah(nn.Module):
    def __init__(self, in_size, out_size, etypes):
        super(HeteroRGCNLayer, self).__init__()
        self.weight = nn.ModuleDict({
            name: nn.Linear(in_size, out_size) for name in etypes
    ...
    def forward(self, G, feat_dict):
        ...
        # TODO: Your task is to implement normalization to node features after message passing!
        # Look at the existing forward function and add your own custom functionality.
        # It could be a custom transformation or message passing mechanism that enhances the layer.
        #HINT: Iterate through G.ntype if node type is 'h' normalize it using L2 Norm (i.e. F.normalize)
